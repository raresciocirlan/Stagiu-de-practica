...
int main()
{
    ...
    char buff[10]; //Buffer de memorie
    ...
    file.seekg(0);
    //Pentru marimea fisierului

    DLT_LOG(flst_example2, DLT_LOG_INFO, DLT_STRING("FLST"), DLT_INT(id),DLT_STRING("file.txt"), DLT_INT(file_size));

    while(file.eof() != true)
    {
        file.read(buff,10);//Se citeste fisierul cu ajutorul unui buffer de 10 bytes

        //Buffer-ul se trimite la dlt
